# Copilot Instructions for PoC Auth Architecture

## Project Overview

This is a proof-of-concept monorepo demonstrating three authentication patterns using a shared OIDC server. See `README.md` for architecture diagrams, component descriptions, and acceptance criteria.

## Quick Reference

- **Spec**: `PoC.md` - Original requirements and acceptance criteria
- **Docs**: `README.md` - Architecture, setup instructions, troubleshooting
...
| Component | Stack |
|-----------|-------|
| auth-server | Node.js + `oidc-provider` + Express |
| api-server | Node.js + Fastify + `jose` (JWT) |
| app-bff | Fastify backend + React/Vite frontend |
| app-spa | React/Vite with custom PKCE |
| app-web | Fastify + EJS templates |
...
./scripts/start.sh    # Start all services (auto-kills existing)
```

## Key Implementation Details

### OIDC Provider (auth-server)
- Configuration in `auth-server/src/config.js`
- Three clients: `app-bff`, `app-spa`, `app-web`
- Resource indicators configured for JWT access tokens
- `conformIdTokenClaims: false` to include profile/email in ID tokens

### JWT Validation (api-server)
- Validates against JWKS at `http://localhost:4000/jwks`
- Audience: `http://localhost:5001`
- Uses `jose` library for validation

### Auth Patterns
1. **BFF** (app-bff): Tokens server-side only, HttpOnly session cookie
2. **SPA** (app-spa): PKCE flow, tokens in React state (memory)
3. **Web** (app-web): Server-rendered, session-based, protected routes

## Code Conventions

- Use async/await over callbacks
- Fastify for new backend services
- React functional components with hooks
- No tokens in localStorage/sessionStorage (except PKCE state temporarily)

## Common Gotchas

1. **Port 5000**: Avoided due to macOS AirPlay conflict (using 5001)
2. **CORS**: SPA needs `clientBasedCORS` in auth-server config
3. **React StrictMode**: Use `useRef` to prevent double-processing in callbacks
4. **Session storage**: Only for PKCE verifier/state during auth flow, cleared after use
